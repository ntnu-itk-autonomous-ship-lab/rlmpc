...
struct ShipModelParams {
    pub draft: f64,
    pub length: f64,
    pub width: f64,
    pub l_r: f64,
    pub M_inv: Matrix3<f64>,
    pub M_rb: Matrix3<f64>,
    pub M_a: Matrix3<f64>,
    pub D_c: Matrix3<f64>,
    pub D_q: Matrix3<f64>,
    pub D_l: Matrix3<f64>,
...
impl ShipModelParams {
    pub fn new() -> Self {
        ...
        Self {
            ...
            l_r: 4.0,
            M_inv: M_inv,
            M_rb: Matrix3::from_partial_diagonal(&[3980.0, 3980.0, 19703.0]),
            M_a: Matrix3::zeros(),
...
impl ShipModel {
    ...
    pub fn dynamics(&self, xs: Vector6<f64>, tau: Vector3<f64>) -> Vector6<f64> {
        let eta = xs.fixed_rows::<3>(0);
        let nu = xs.fixed_rows::<3>(3);

        let eta_dot = self.Rpsi(eta) * nu;
        let nu_dot = self.params.M_inv * (tau - self.C(eta, nu) * nu - self.D(eta, nu) * nu);
        let mut xs_dot = Vector6::zeros();
