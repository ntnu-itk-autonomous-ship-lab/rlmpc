...
class Encoder(nn.Module):
    ...
    def __init__(self, channels: int, hidden_dim: int, latent_dim: int):
        ...
        self.to_latent = nn.Sequential(
            nn.Conv2d(hidden_dim, latent_dim, 1, 1, 0, bias=True), nn.BatchNorm2d(latent_dim), nn.ReLU()
        )
...
# class VectorQuantizer(nn.Module):
#     def __init__(self, num_embeddings, hidden_dim):
#         super().__init__()

#         self.embed_space = nn.Embedding(num_embeddings, hidden_dim)  # "$e \in \mathbb{R}^{K \times D}$"
#         self.embed_space.weight.data.uniform_(-1 / num_embeddings, 1 / num_embeddings)  # Uniform distribution??

#     def vector_quantize(self, x):  # (b, `num_embeddings`, h, w)
#         b, _, h, w = x.shape
...
class Decoder(nn.Module):
    def __init__(self, channels: int, hidden_dim: int, latent_dim: int):
        ...
        self.res_block = nn.Sequential(
            ...
            nn.BatchNorm2d(hidden_dim),
            nn.ReLU(),
            nn.Conv2d(hidden_dim, latent_dim, 1, 1, 0, bias=False),
        ...
        self.conv_block = nn.Sequential(
            nn.BatchNorm2d(latent_dim),
            nn.ReLU(),
            nn.ConvTranspose2d(latent_dim, hidden_dim, 4, 2, 1, bias=False),
            nn.BatchNorm2d(hidden_dim),
            nn.ReLU(),
